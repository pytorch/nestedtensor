...
class TestFunctional(TestCase):
    ...
    def _test_mha_impl(self, self_attention, device):
        embed_dim = 2
        num_heads = 2
    ...
    def test_mha(self):
        for self_attention in (True, False):
            self._test_mha_impl(self_attention, torch.device('cpu'))

    @torch.inference_mode()
    @unittest.skipIf(not torch.cuda.is_available(), "Test requires cuda")
    def test_mha_cuda(self):
        for self_attention in (True, False):
            self._test_mha_impl(self_attention, torch.device('cuda'))

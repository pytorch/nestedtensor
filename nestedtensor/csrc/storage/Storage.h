...
struct NestedTensorStorage {
  ...
  explicit NestedTensorStorage(ListStorage&& list_storage)
      : _list_storage(list_storage), _kind(NestedTensorStorageKind::list) {}
  int64_t dim() const {
    switch (_kind) {
      case packed:
  ...
  bool is_pinned() const {
    switch (_kind) {
      case packed:
        ...
        break;
      case list:
        return _list_storage->is_pinned();
        break;
    }
  }
  const SizeNode nested_size() const {
    switch (_kind) {
      case packed:
        ...
        break;
      case list:
        return _list_storage->nested_size();
        break;
    }
  }
  const SizeNode nested_stride() const {
    switch (_kind) {
      case packed:
        ...
        break;
      case list:
        return _list_storage->nested_stride();
        break;
    }
  }
  const std::vector<c10::optional<int64_t>> opt_sizes() const {
    switch (_kind) {
      case packed:
        ...
        break;
      case list:
        return _list_storage->opt_sizes();
        break;
    }
  }

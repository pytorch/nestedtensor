...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  // via unbind.
  //  py::class_<torch::nested_tensor::THP_ListNestedTensor>(m,
  //  "_ListNestedTensor")
  ...
  //          })
  //      .def("backward",
  //      &torch::nested_tensor::THP_ListNestedTensor::backward) .def(
  //          "element_size",
  ...
  //      .def("is_pinned",
  //      &torch::nested_tensor::THP_ListNestedTensor::is_pinned) .def("dim",
  //      &torch::nested_tensor::THP_ListNestedTensor::dim) .def(
  //          "nested_size",
  ...
  //      &torch::nested_tensor::THP_ListNestedTensor::grad) .def("detach",
  //      &torch::nested_tensor::THP_ListNestedTensor::detach) .def(
  //          "requires_grad_",
  //          &torch::nested_tensor::THP_ListNestedTensor::requires_grad_)
  //      .def("to_tensor",
  //      &torch::nested_tensor::THP_ListNestedTensor::to_tensor)
  ...
  // since you can expect transparent changes to the constiuents
  // via unbind.
  py::class_<torch::nested_tensor::THPNestedTensor>(m, "_BufferNestedTensor")
      ...
      .def(
          ...
          [](torch::nested_tensor::THPNestedTensor self) {
            ...
            //       self.data().get_buffer(), c10::IntArrayRef(split_sizes),
            //       0);
            //   for (int64_t i = 0; i < self.len(); i++) {
            //     result.push_back(
